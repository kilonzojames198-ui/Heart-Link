...
exports.getDashboard = async (req, res) => {
  ...
  const swiped = db.prepare('SELECT swiped_id FROM swipes WHERE swiper_id=?').all(uid).map(r=>r.swiped_id);
  swiped.push(uid);
  const ph = swiped.map(()=>'?').join(',');
  let sql = `SELECT * FROM users WHERE id NOT IN (${ph}) AND role!='admin' AND is_active=1`;
  const params = [...swiped];
  if (me.interested_in !== 'everyone') { sql += ' AND gender=?'; params.push(me.interested_in); }
  sql += ' ORDER BY RANDOM() LIMIT 1';
  const profile = db.prepare(sql).get(params);
  if (profile) {
...
exports.postSwipe = async (req, res) => {
  ...
  db.prepare('INSERT OR IGNORE INTO swipes (swiper_id,swiped_id,action) VALUES (?,?,?)').run(uid, parseInt(swiped_id), action);

  if (action === 'like' || action === 'superlike') {
    const their = db.prepare('SELECT action FROM swipes WHERE swiper_id=? AND swiped_id=?').get(parseInt(swiped_id), uid);
    if (their && (their.action==='like'||their.action==='superlike')) {
      ...
      const matched = db.prepare('SELECT name FROM users WHERE id=?').get(parseInt(swiped_id));
