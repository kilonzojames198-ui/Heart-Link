...
exports.updateProfile = async (req, res) => {
  ...
  db.prepare(`UPDATE users SET name=?,age=?,gender=?,interested_in=?,bio=?,interests=?,avatar=? WHERE id=?`)
    .run(name.trim(),parseInt(age),gender,interested_in||'everyone',bio||'',interestsJson,avatar,req.session.userId);
